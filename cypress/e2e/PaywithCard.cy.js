...
describe('Pay with Card', () => {
  ...
  it('should login successfully and pay with card', () => {
   ...
   //cy.get(':nth-child(4) > .css-t5rqjb > .sc-a1e2bb0b-17').clear().type('Test Order');
    ...
    cy.get('.chakra-stack > :nth-child(1) > .chakra-radio > .chakra-radio__control').click()
    cy.get('input[name="cardNumber"]').click().type(cardDetails.card1.cardNumber)
    cy.get('input[name="cardHolderName"]').click().type(cardDetails.card1.cardholderName)
    // Select month (e.g., selecting "March")
    cy.get(':nth-child(1) > .sc-a1e2bb0b-14 > .chakra-select__wrapper > .chakra-select').select('03'); // Index 0 corresponds to the first select element

    // Select year (e.g., selecting "2024")
    cy.get('.css-1q3zom8 > :nth-child(2) > .sc-a1e2bb0b-14 > .chakra-select__wrapper > .chakra-select').select('2024'); // Index 1 corresponds to the second select element
    cy.get('input[name="cvc"]').type(cardDetails.card1.cvv); 

    //Clcik on Confrim Order
    cy.get('.css-gq7ao3 > .chakra-button').should('be.visible').click()

      // After confirming the order, verify the URL
      cy.url().should('eq', 'https://www.rocabalighting.co.uk/checkout/3dsecure');
      const iframeSelector = 'iframe#secureIFrame2'; // Selector for the iframe
      const elementToFindSelector = 'input[type="text"]'; 

        // Wait for the iframe to load
        cy.frameLoaded(iframeSelector);

        // Switch to the iframe context and interact with elements inside
        cy.iframe(iframeSelector)
            .find(elementToFindSelector)
            .should('be.visible')
            .click()
            .type('challenge');

        // Optionally, interact with other elements inside the iframe
        cy.iframe(iframeSelector)
            .find('.button4')
            .should('be.visible')
            .click();
